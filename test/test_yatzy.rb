...
class YatzyTest < Test::Unit::TestCase
  ...
  def test_1s_scores
    assert_equal 0, Yatzy.sum_number(Dice.new(6, 2, 2, 4, 5).faces, 1)
    assert_equal 1, Yatzy.sum_number(Dice.new(1, 2, 3, 4, 5).faces, 1)
    assert_equal 2, Yatzy.sum_number(Dice.new(1, 2, 1, 4, 5).faces, 1)
    assert_equal 4, Yatzy.sum_number(Dice.new(1, 2, 1, 1, 1).faces, 1)
  end

  def test_2s_scores
    assert_equal 4, Yatzy.sum_number(Dice.new(1, 2, 3, 2, 6).faces, 2)
    assert_equal 10, Yatzy.sum_number(Dice.new(2, 2, 2, 2, 2).faces, 2)
  end

  def test_threes_scores
    assert_equal 6, Yatzy.sum_number(Dice.new(1, 2, 3, 2, 3).faces, 3)
    assert_equal 12, Yatzy.sum_number(Dice.new(2, 3, 3, 3, 3).faces, 3)
  end

  def test_fours_scores
    assert_equal 12, Yatzy.sum_number(Dice.new(4, 4, 4, 5, 5).faces, 4)
    assert_equal 8, Yatzy.sum_number(Dice.new(4, 4, 5, 5, 5).faces, 4)
    assert_equal 4, Yatzy.sum_number(Dice.new(4, 5, 5, 5, 5).faces, 4)
  end

  def test_fives_scores
    assert_equal 10, Yatzy.sum_number(Dice.new(4, 4, 4, 5, 5).faces, 5)
    assert_equal 15, Yatzy.sum_number(Dice.new(4, 4, 5, 5, 5).faces, 5)
    assert_equal 20, Yatzy.sum_number(Dice.new(4, 5, 5, 5, 5).faces, 5)
  end

  def test_sixes_scores
    assert_equal 0, Yatzy.sum_number(Dice.new(4, 4, 4, 5, 5).faces, 6)
    assert_equal 6, Yatzy.sum_number(Dice.new(4, 4, 6, 5, 5).faces, 6)
    assert_equal 18, Yatzy.sum_number(Dice.new(6, 5, 6, 6, 5).faces, 6)
  end
  ...
  def test_two_pair_scores
    assert_equal 16, Yatzy.score_two_pair(Dice.new(3, 3, 5, 4, 5).faces)
    assert_equal 16, Yatzy.score_two_pair(Dice.new(3, 3, 5, 5, 5).faces)
  end
